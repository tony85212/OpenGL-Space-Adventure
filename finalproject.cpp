...
const char* WINDOWTITLE = { "OpenGL / GLUT Final Project -- Yu-Hsiang Liu" };
...
Display()
{
    ...
    */
    ...
    glLoadIdentity();
    glColor3f(1., 1., 1.);
    DoRasterString(5., 5., 0., (char*)"Final Project - Yu-Hsiang Liu");
    ...
    ms = timelimit - glutGet(GLUT_ELAPSED_TIME);
    if (ms > 0)
    {
        d = sqrt((cam.ex - bx) * (cam.ex - bx) + (cam.ey - by) * (cam.ey - by) + (cam.ez - bz) * (cam.ez - bz));
        //printf("%f\n", d);
        s = ms / 1000;
        ms %= 1000;
        ms /= 10;
        min = s / 60;
        s %= 60;
        sprintf(str, "%d : %d : %d", min, s, ms);
        DoRasterString(5., 92., 0., str);
        if (d < 1)
        {
            timelimit = 0;
            d = 0;
    ...
    else
    {
        tt = true;
        if (mission == true)
            DoRasterString(5., 92., 0., "Mission Complete!");
        else
            DoRasterString(5., 92., 0., "Mission Failed!");
    }
...
InitMenus()
{
    ...
    glutAddMenuEntry("Inside Ship", INSIDE);
    glutAddMenuEntry("Outside Space", OUTSIDE);
...
InitLists()
{
    ...
    //Broken Ship
    BrokenShip = glGenLists(1);
    glNewList(BrokenShip, GL_COMPILE);
    glEnable(GL_LIGHTING);
    glPushMatrix();
    glTranslatef(bx, by, bz);
    SetMaterial(0.75, 0.2, 1, 2);
    //MjbSphere(0.4, 100, 100);
    glScalef(0.01, 0.01, 0.01);
    LoadObjFile("img/ship.obj");
    glDisable(GL_LIGHTING);
    glPopMatrix();
    glEndList();
    ...
    glEnable(GL_LIGHTING);
    glPushMatrix();
    SetMaterial(0.75, 0.75, 0.75, 2);
    //MjbSphere(2, 100, 100);
    glRotatef(270, 0, 1, 0);
    glScalef(0.01, 0.01, 0.01);
    LoadObjFile("img/warpship.obj");
    glDisable(GL_LIGHTING);
    glPopMatrix();
    glEndList();
